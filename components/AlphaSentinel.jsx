...
import { Brain, Activity, ShieldAlert, Cpu, Radio, Loader2 } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import axios from 'axios';

export default function AlphaSentinel({ activeInsight, selectedAsset }) {
  ...
  useEffect(() => {
    ...
    if (cache[selectedAsset]) {
      setAnalysis(cache[selectedAsset]);
      return;
    }
    ...
    const fetchAnalysis = async () => {
      setLoading(true);
      try {
        const res = await axios.get(`/api/market/analysis?symbol=${encodeURIComponent(selectedAsset)}`);
        setAnalysis(res.data);
        setCache(prev => ({ ...prev, [selectedAsset]: res.data }));
      } catch (err) {
        console.error("Failed to fetch asset analysis", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const getInsightData = () => {
    // If user is hovering over something, show the hover insight
    if (activeInsight && activeInsight !== 'IDLE') {
    ...
    // Else if we have Gemini analysis, show it!
    if (analysis) {
      return analysis;
    }
  ...
  return (
    <div className="glass-panel border-[#FFBF00]/20 overflow-hidden bg-[#030307]/90 backdrop-blur-xl relative flex flex-col p-5 gap-4 h-full min-h-[350px]">
      ...
      <div className="relative flex-1 flex flex-col justify-center bg-black/40 border border-white/5 p-4 rounded-lg overflow-hidden min-h-[140px]">
        <div className="absolute inset-0 bg-gradient-to-b from-[#FFBF00]/2 to-transparent pointer-events-none" />
        
        {loading && activeInsight === 'IDLE' ? (
          <div className="flex flex-col items-center justify-center gap-3 py-6 z-10">
            <Loader2 className="animate-spin text-[#FFBF00]" size={20} />
            <span className="text-[8px] font-mono text-gray-500 uppercase tracking-widest font-black">Synthesizing Gemini Insights for {selectedAsset}...</span>
          </div>
        ) : (
          <AnimatePresence mode="wait">
            <motion.div
              key={activeInsight + "_" + (data.title || "idle")}
